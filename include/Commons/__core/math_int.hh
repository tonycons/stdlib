...
///
/// Lookup tables for particular base-N representations
///
...
struct IntHelper
{
    ...
    ///
    FORCEINLINE constexpr static bool addOverflow(T const& x, T const& y, T& res)
    {
    ...
    /// @return If overflow occurred
    ///
    FORCEINLINE constexpr static bool addOverflow(T const& x, T const& y)
    {
        T res{};
    ...
    /// @return If overflow occurred
    ///
    FORCEINLINE constexpr static bool mulOverflow(T const& x, T const& y, T& res)
    {
    ...
    /// @return If overflow occurred
    ///
    FORCEINLINE constexpr static bool mulOverflow(T const& x, T const& y)
    {
        T res{};
    ...
    ///
    template<T Base>
    constexpr static Optional<T> pow(T const& k)
    {
        constexpr static PowerLookupTable<Base> table;
    ...
    /// @param x the value
    ///
    constexpr static auto clz(T const& x) -> UintRanged<BITS>
    {
        using R = UintRanged<BITS>;
        if constexpr (sizeof(x) <= sizeof(int)) {
            ...
            constexpr auto bits = R(sizeof(T)) * 8u;
            R count = 0;
            if (!x) {
                return bits;
            }
    ...
    ///
    template<T Base = 10>
    constexpr static u8 log(T const& x)
    {
        if constexpr (Base == 2) {
    ...
    constexpr static T msd(T const& x)
    {
    ...
    ///
    /// Converts a value of this integer type into a string.
    /// @param value The value
    /// @param writer A function that sequentially receives each character in the string representation of value.
    ///
    template<IntegerParsingScheme S = IntegerParsingScheme::DEFAULT>
    constexpr static void outputString(T value, auto const& writer)
    {
        if constexpr (IsIntegerSigned<T>) {
            if (value < 0) {
                writer('-');
        ...
        do {
            writer(char(msd<10>(value) + '0'));
            value /= 10;
        } while (value != 0);
    }

    ///
    /// Converts a value of this integer type into a string using a particular base-N representation.
    /// @tparam Base The base-N representation
    /// @param value The value
    /// @param writer A function that sequentially receives each character in the string representation of value.
    ///
    template<IntBaseFmt Base, IntegerParsingScheme S = IntegerParsingScheme::DEFAULT>
    constexpr static void outputStringBase(T value, auto const& writer)
    {
        if constexpr (S == IntegerParsingScheme::JSON) {
...
struct Int : public IntHelper<int> {} inline constexpr Int;
struct Uint : public IntHelper<unsigned> {} inline constexpr Uint;
struct Usize : public IntHelper<usize> {} inline constexpr Usize;
struct Isize : public IntHelper<isize> {} inline constexpr Isize;

struct U8  : public IntHelper<u8>  {} inline constexpr U8;
struct I8  : public IntHelper<i8>  {} inline constexpr I8;
struct U16 : public IntHelper<u16> {} inline constexpr U16;
struct I16 : public IntHelper<i16> {} inline constexpr I16;
struct U32 : public IntHelper<u32> {} inline constexpr U32;
struct I32 : public IntHelper<i32> {} inline constexpr I32;
struct U64 : public IntHelper<u64> {} inline constexpr U64;
struct I64 : public IntHelper<i64> {} inline constexpr I64;

// clang-format on


struct _u128_c : public IntHelper<u128>
{
    constexpr static u128 operator()(auto x) { return parse(x); }
} inline constexpr U128;

struct _i128_c : public IntHelper<i128>
{
    constexpr static i128 operator()(auto x) { return parse(x); }
} inline constexpr I128;

struct _u256_c : public IntHelper<u256>
{
    constexpr static u256 operator()(auto x) { return parse(x); }
} inline constexpr U256;

struct _i256_c : public IntHelper<i256>
{
    constexpr static i256 operator()(auto x) { return parse(x); }
} inline constexpr I256;
...
struct Bool
{
    void outputString(bool v, auto const& out)
    {
        if (v) {
            out('t');
            out('r');
