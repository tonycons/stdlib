...
/// For example, Index -1 refers to the last element.
///
struct Index : Union<isize, usize>
{
    ...
    ///
    NODISCARD FORCEINLINE constexpr auto computeUnchecked(auto const& array) const
    {
        usize i = match(
            [&](usize k) { return k; },
            [&](isize k) {  // Possible negative index
                return k < 0 ? usize(isize(array.length()) + k) : usize(k);
            });
        return i;
    }
    ...
    /// @param a The array to compute the index for.
    ///
    NODISCARD FORCEINLINE constexpr auto compute(auto const& array) const
    {
        usize i = computeUnchecked(array);
        Assert(i < array.length(), ASSERT_MSG_BOUNDS);
        return i;
    }
